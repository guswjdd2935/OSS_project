    ...
    try:
        ...
        if filter_result[0] != -1 and filter_result[1] != -1:
            if filter_result[1] > 100:  # 공이 가까워진 경우
                ...
                # 정면(각도 0)으로 맞춘 뒤, 터치 센서가 눌릴 때까지 후진
                turn(0, 100)  # 다시 각도 0으로 보정
                while not touch_sensor.pressed():  # 터치 센서가 눌릴 때까지 반복
                    robot.drive(-100, 0)          # 후진 (속도 -100, 회전 0)
                robot.stop()                      # 터치 센서가 눌리면 멈춤
